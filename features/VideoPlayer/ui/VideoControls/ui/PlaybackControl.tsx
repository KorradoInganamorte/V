...
export const PlaybackControl = () => {
  ...
  return (
    <View className={`${isFullscreen ? "h-16 w-16" : "h-12 w-12"} rounded-full bg-black/40`}>
      {status === "readyToPlay" ? (
        isPlaying ? (
          <TouchableOpacity onPress={onPlaybackPress} className="flex h-full w-full items-center justify-center">
        ...
        isEnded ? (
          <TouchableOpacity onPress={onReplayPress} className="flex h-full w-full items-center justify-center">
